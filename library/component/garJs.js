...
(function ($) {
    ...
    fn.createImg=function () {
        var img,self=this,l=len=this.ITEMSNUM;
    ...
    fn.startMove=function () {
        ...
        gar.reverseEach(this.items,function (i,item,len) {
            ...
            if(item.angle%45===0&&item.angle%90!=0&&self.keyOne){
                ...
                self.ctx.fillRect(item.x,item.y+5,2,item.img.height-15);
                self.ctx.globalCompositeOperation='destination-over';
                var n=0;
                setInterval(function () {
                    //self.ctx.clearRect(item.x+5*(n-1),item.y+5,5,item.img.height-15);
                    self.ctx.clearRect(0,0,self.WIDTH,self.HEIGHT);
                    self.calcPos(item,self,item.img.width,item.img.height);

                    self.ctx.globalCompositeOperation='source-over';

                    self.ctx.fillRect(item.x+5*n,item.y+5,2,item.img.height-15);
    ...
    fn.calcPos=function (info,self,_w,_h) {
        ...
        self.ctx.drawImage(info.img,info.x,info.y,_w*num,_h*num);
