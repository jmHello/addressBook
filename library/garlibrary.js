(function (window) {
    ...
    gar.addHandler=function (ele, type, handler,bubble) {
        if(ele.addEventListener)this.addHandler=function (ele, type, handler, bubble) {
            ele.addEventListener(type,handler,bubble);
        };
        else if(ele.attachEvent)this.addHandler=function (ele, type, handler) {
            ele.attachEvent('on'+type,handler);
        };
        else this.addHandler=function (ele, type, handler) {
                ele['on'+type]=handler;
            };
    ...
    gar.removeHandler=function (ele, type, handler, bubble) {
        if(ele.removeEventListener)this.removeHandler=function (ele, type, handler, bubble) {
            ele.removeEventListener(type,handler,bubble);
        };
        else if(ele.detachEvent)this.removeHandler=function (ele, type, handler) {
            ele.detachEvent(type,handler,bubble);
        };
        else this.removeHandler=function (ele, type) {
                ele['on'+type]=null;
            };
    ...
    gar.each=function (obj, callback) {
        ...
        else for(o in obj)val=callback.call(obj[o],i,obj[o],l);
        return obj;
    };
...
//for count:
    ...
    gar.binarySearch=function (arr, dest, start, end) {
        ...
        while(start<=end){
            mid=(start+end)>>1;
            ele=arr[mid];
            if(ele<item)low=mid+1;
            else if(ele>item)high=mid-1;
            else return mid;
        }
        return -1;
    };
